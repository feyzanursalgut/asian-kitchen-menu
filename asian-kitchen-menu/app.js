const menu = [
  ...
  // Tüm yemeklerin gösterilmesini sağlayan fonksiyon
  const getFood = () => {
    let allFoods = "";
  ...
  // sadece kore yemeklerini get ettik.
  const getKorea = () => {
    let koreaFoods = "";
  ...
  // sadece Japon yemeklerini get ettik.
  const getJapan = () => {
    let japanFoods = "";
  
    menu.map((food) => {
      if (food.category === "Japan") {
        japanFoods += postFoods(food);
      }
    });
    menuList.innerHTML = japanFoods;
  };
  
  // sadece çin yemeklerini get ettik.
  const getChina = () => {
    let chinesefoods = "";
  
    menu.map((food) => {
      if (food.category === "China") {
        chinesefoods += postFoods(food);
      }
    });
    menuList.innerHTML = chinesefoods;
  };
  
//   butonları aktif etme ve tüm yemekleri gösterme
